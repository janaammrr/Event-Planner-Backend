...
func InviteUserToEvent(c *gin.Context) {
	...
	if err := config.DB.Where("event_id = ? AND user_id = ? AND role = ?", eventID, userID, "organizer").
		...
		utils.JSONError(c, http.StatusInternalServerError, "failed to verify permissions: "+err.Error())
		return
	}

	// Find user by email
	var invitedUser models.User
	if err := config.DB.Where("email = ?", req.Email).First(&invitedUser).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
	...
	if err := config.DB.Where("event_id = ? AND user_id = ?", eventID, invitedUser.ID).
	...
	if err := config.DB.Create(newAttendee).Error; err != nil {
...
func DeleteEvent(c *gin.Context) {
	...
	if event.CreatedBy != userID {
		utils.JSONError(c, http.StatusForbidden, "only the event creator can delete the event")
		return
	}

	// Delete event (cascade will handle attendees and tasks)
...
func GetEventDetails(c *gin.Context) {
	...
	if userAttendee.UserID != 0 {
		response["myRole"] = userAttendee.Role
		response["myStatus"] = userAttendee.Status
