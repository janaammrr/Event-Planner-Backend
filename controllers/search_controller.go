...
func searchEvents(userID uint, req SearchRequest) ([]gin.H, error) {
	...
	for i, event := range events {
		// Find user's role and status for this event
		var userAttendee models.EventAttendee
		config.DB.Where("event_id = ? AND user_id = ?", event.ID, userID).First(&userAttendee)

		result[i] = formatEventResponse(event)
		if userAttendee.UserID != 0 {
			result[i]["myRole"] = userAttendee.Role
			result[i]["myStatus"] = userAttendee.Status
